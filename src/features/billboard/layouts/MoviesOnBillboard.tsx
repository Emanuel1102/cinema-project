...
import { dateKey, movies, next7Days, type Movie } from "@/lib/data";
...
export const MoviesOnBillboard = () => {
  ...
  const cityLabel = stateCity ? formatLocationLabel(stateCity) : "Bogotá";
  ...
  function handleReserve(movie: Movie) {
    ...
    const showtime = movie.showtimes.find((item) => item.seatsLeft > 0);
    if (!showtime) {
      toast.error("No hay funciones disponibles para esta película.");
      return;
    }
    ...
    toast.success(`Reserva simulada para ${movie.title}.`);
  }
            ...
            <div className="relative z-10 flex min-h-[480px] items-end px-6 py-8 sm:px-8 lg:px-12">
              <div className="max-w-2xl">
                ...
                <div className="mt-7 flex flex-wrap gap-4">
                  <button onClick={() => handleTrailer(featuredMovie)} className="rounded-xl bg-gradient-to-r from-[#7C3AED] via-[#818CF8] to-[#DB2777] px-5 py-3 text-sm font-bold uppercase tracking-[0.16em] text-white shadow-lg shadow-[#7C3AED]/25 transition duration-200 hover:brightness-110 hover:scale-[1.02]">Ver tráiler</button>
                  <button onClick={() => handleReserve(featuredMovie)} className="rounded-xl border border-white/15 bg-white/5 px-5 py-3 text-sm font-bold uppercase tracking-[0.16em] text-slate-100 transition duration-200 hover:bg-white/10 hover:scale-[1.02]">Reservar</button>
                </div>
              </div>
            </div>
          </div>
              ...
              <article key={movie.id} className="group overflow-hidden rounded-[22px] border border-white/10 bg-[#111827] shadow-[0_16px_40px_rgba(15,23,42,0.55)] transition duration-200 hover:-translate-y-1 hover:border-[#818CF8]/40">
                ...
                <div className="space-y-4 p-4">
                  ...
                  <div className="flex gap-2 pt-1">
                    <Link to={`/movies/${movie.id}`} className="flex-1 rounded-xl border border-white/10 bg-slate-800 px-3 py-2 text-center text-xs font-bold text-slate-200 transition duration-200 hover:bg-slate-700 hover:scale-[1.02]">Ver detalle</Link>
                    <button onClick={() => handleReserve(movie)} className="flex-1 rounded-xl bg-[#7C3AED] px-3 py-2 text-xs font-bold text-white transition duration-200 hover:opacity-90 hover:scale-[1.02]">Reservar</button>
                  </div>
                </div>
              </article>
            ))}
