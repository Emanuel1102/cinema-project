...
export const SeatItem = ({ seat, isSelected = false, onSelect }: SeatItemProps) => {
  // La prop isSelected controla estrictamente el estado visual
  const state: SeatStatus = isSelected ? "selected" : seat.status;
  ...
  return (
    <button
      ...
      className={[
        ...
        !isSelected && seat.status === "available" && getSeatVisualClassName(seat.type),
